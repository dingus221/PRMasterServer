...
class SBClient:
            ...
            while len(self.__readbuffer) > 0:
                ...
                if packet[2] == '\x00':  #List request
                    if len(packet) > 25:
                        ...
                        self.outCrypt.SBCryptStart(bytearray(qfromkey), bytearray(cchallenge),
                    ...
                    print
                    "SB recieved unknown command" + str(ord(packet[2]));
    ...
    def disconnect(self, quitmsg):
        print
        'SBClient disconnected(' + self.host + ':' + str(self.port) + '). ' + str(quitmsg)
        self.socket.close()
...
class SBQRServer:
    ...
    def print_hostlist(self):
        for y, x in enumerate(self.Hostlist):
            print
            str(y) + '. ' + self.Hostlist[x].ip + ':' + str(self.Hostlist[x].port) + '(' + self.Hostlist[
                x].sessionid + ') ' + str(self.Hostlist[x].lastheard)
            print
            x.data
            print
            ' * * * '
    ...
    def qr_forw_to(self, rawdata):
        if rawdata[9:15] == '\xfd\xfc\x1e\x66\x6a\xb2':
            ...
            if (ip, port) in self.Hostlist:
                print
                "forwarding to existing host"
            else:
                print
                "forwarding to unknown address"
            resp = '\xfe\xfd\x06'
            if (ip, port) in self.Hostlist:
        ...
        while True:
            ...
            for x in rlst:
                if x == self.qr:
                    ...
                    print
                    'accepted sb connection from %s:%s.' % (addr[0], addr[1])
            for x in wlst:
...
def Main():
    ...
    for x in xrange(0, 60):
        str1 = ' '.join([junk1[random.randrange(0, len(junk1))] for x in xrange(0, random.randrange(0, 3))] + [
            junk0[random.randrange(0, len(junk0))]] + [junk1[random.randrange(0, len(junk1))] for x in
                                                       xrange(0, random.randrange(0, 3))])
